...
if (!UserInGroup("tweakparams")) {
    ...
    print "ですから、パラメータを編集することはできません。\n";
    PutFooter();
    exit;
}


PutHeader("新しいパラメータを保存する");
...
#    print "Processing $i...<BR>\n";
    ...
    if ($::FORM{$i} ne Param($i)) {
        if (defined $::param_checker{$i}) {
            ...
            my $ok = &$ref($::FORM{$i});
            if ($ok ne "") {
                print "$i の新しい値は正しくありません。: $ok<p>\n";
                print "<b>Back</b> を押して、もう一度ためしてみてください。\n";
                PutFooter();
                exit;
            }
        }
...
print "OK、完了です。<p>\n";
print "<a href=editparams.cgi>パラメータをさらに編集する</a><p>\n";
print "<a href=query.cgi>検索ページに戻る</a>\n";
