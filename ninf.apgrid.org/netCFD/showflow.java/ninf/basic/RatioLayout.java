...
public class RatioLayout implements LayoutManager, java.io.Serializable {
    double ratio[];
    int mode;
    ...
    public RatioLayout(int mode, double ratio[]) {
      this.mode = mode;
      this.ratio = ratio;
    }
	...
	if (mode == HORIZONTAL)
	  w -= gaps * (ncomponents - 1);
	else
	  h -= gaps * (ncomponents - 1);
	double sum = 0.0;
	int x0 = insets.left;
	int y0 = insets.top;
	if (mode == HORIZONTAL){
	  for (int i = 0; i < ncomponents; i++){
	    try {
	      ...
	      parent.getComponent(i).reshape(x0 + (int)sum, y0, tmpW, h);
	      sum += tmpW + gaps;
	    } catch (ArrayIndexOutOfBoundsException e){
	    }
	  }
	  ...
	  for (int i = 0; i < ncomponents; i++){
	    try {
	      ...
	      parent.getComponent(i).reshape(x0, y0+(int)sum, w, tmpH);
	      sum += tmpH + gaps;
	    } catch (ArrayIndexOutOfBoundsException e){
	    }
	  }
