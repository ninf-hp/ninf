...
if(Param("usebuggroupsentry") && GroupExists($product)) {
    if(!UserInGroup($product)) {
        DisplayError("申しわけありません。あなたにはこのプロダクトのバグを登録する権限がありません。", "Permission Denied");
        exit;
    }
}

if (!$::FORM{'component'}) {
    DisplayError("このバグに関係するコンポーネントを選択してください。分からなければ、これと思うもので結構です。");
    exit;                  
}

if (!defined $::FORM{'short_desc'} || trim($::FORM{'short_desc'}) eq "") {
    DisplayError("バグの要約を記入してください。");
    exit;
}
