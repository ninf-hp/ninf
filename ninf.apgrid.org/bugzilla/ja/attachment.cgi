...
sub validateID
{
    ...
    detaint_natural($::FORM{'id'})
        || DisplayError("正しい添付ファイル番号を入力してください。") 
      && exit;
  
  # Make sure the attachment exists in the database.
  SendSQL("SELECT bug_id FROM attachments WHERE attach_id = $::FORM{'id'}");
  MoreSQLData()
    || DisplayError("添付ファイル #$::FORM{'id'} は存在しません。") 
    && exit;
...
sub validateCanEdit
{
    ...
    FetchSQLData()
      || DisplayError("添付ファイル #$attach_id を編集する権限がありません。")
      && exit;
}
...
{
  $::FORM{'description'}
    || DisplayError("添付ファイルの説明を入力してください。")
      && exit;
}
...
sub validateContentType
{
  ...
  {
    DisplayError("ファイル形式を決める方法を、<em>自動認識</em>, <em>リストから選ぶ</em>, or <em>手動で入力する</em> から選んでください。");
    exit;
  }
  elsif ($::FORM{'contenttypemethod'} eq 'autodetect')
  {
    ...
    {
      DisplayError("あなたは Bugzilla にファイル形式の自動認識を指示しました。しかし、ファイルをアップロードするとき Web ブラウザがファイル形式を特定できませんでした。そのため、ファイル形式を手動で入力してください。");
      exit;
    }
  ...
  else
  {
    ...
    DisplayError("フォームからの送信が、どこか壊れています。
<em>ファイル形式</em>は、<em>自動認識</em>, <em>リストから選ぶ</em>, or <em>手動で入力する</em> のどれかになるはずですが、<em>$htmlcontenttypemethod</em> になっています。");
    exit;
  }
  ...
  if ( $::FORM{'contenttype'} !~ /^(application|audio|image|message|model|multipart|text|video)\/.+$/ )
  {
    ...
    DisplayError("ファイル形式 <em>$htmlcontenttype</em> は正しくありません。正しい形式は <em>foo/bar</em> 形式で、<em>foo</em> の部分は <em>application, audio, image, message, model, multipart, text,</em> または <em>video</em> になります。");
    exit;
  }
}
...
sub validateStatuses
{
  ...
  foreach my $status (@{$::MFORM{'status'}})
  {
    grep($_ == $status, @statusdefs)
      || DisplayError("入力されたステータスの一つが、添付ファイルのステータスとして正しくありません。")
        && exit;
...
sub validateData
{
  ...
  if ( $::FORM{'ispatch'} && $maxpatchsize && $len > $maxpatchsize*1024 )
  {
    ...
    DisplayError("あなたが添付しようとしたファイルは ${lenkb}KB (キロバイト) です。パッチの最大サイズは ${maxpatchsize}KB です。パッチをいくつかに分割してみてください。");
    exit;
  } elsif ( !$::FORM{'ispatch'} && $maxattachmentsize && $len > $maxattachmentsize*1024 ) {
    my $lenkb = sprintf("%.0f", $len/1024);
    DisplayError("あなたが添付しようとしたファイルは ${lenkb}KB (キロバイト) です。パッチ以外の添付ファイルの最大サイズは ${maxpatchsize}KB です。画像であれば JPG や PNG のような圧縮フォーマットに変換するとか、web 上の別の場所に置いて URL 欄に記入する、コメントに記入してリンクするなどを試してみてください。");
    exit;
  }
}
...
{
  defined $::FILE{'data'}
    || DisplayError("添付するファイルが指定されていません。")
      && exit;
}
...
sub validateObsolete
{
  ...
  foreach my $attachid (@{$::MFORM{'obsolete'}}) {
    detaint_natural($attachid)
      || DisplayError("あなたが用済みにしたい添付ファイルの番号のうち一つは、正しくありません。") 
        && exit;
    ...
    # Make sure the attachment exists in the database.
    MoreSQLData()
      || DisplayError("添付ファイル #$attachid は存在しません。") 
        && exit;
    ...
    if ($bugid != $::FORM{'bugid'})
    {
      ...
      DisplayError("添付ファイル #$attachid ($description) は bug #$bugid に添付されています。しかしあなたは、それを用済みにしようとする一方で、bug #$::FORM{'bugid'} にファイルを新しく添付しようとしています。");
      exit;
    }
    ...
    {
      $description = html_quote($description);
      DisplayError("添付ファイル #$attachid ($description) は既に用済みになっています。");
      exit;
    }
