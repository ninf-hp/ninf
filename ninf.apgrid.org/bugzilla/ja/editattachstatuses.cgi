...
UserInGroup("editcomponents")
  || DisplayError("添付ファイルのステータスを管理する権限がありません。")
  && exit;
...
{
  $::FORM{'name'}
    || DisplayError("ステータス名を入力しなくてはなりません。")
      && exit;

  $::FORM{'name'} !~ /[\s,]/
    || DisplayError("ステータス名にはコンマや空白を含むことができません。")
      && exit;

  length($::FORM{'name'}) <= 50
    || DisplayError("ステータス名は 50文字を越えてはいけません。")
      && exit;
}
...
{
  $::FORM{'desc'}
    || DisplayError("ステータスの説明を入力しなくてはなりません。")
      && exit;
}
...
sub validateSortKey
{
  $::FORM{'sortkey'} =~ /^\d+$/
    && $::FORM{'sortkey'} < 32768
      || DisplayError("ソート時のキーは 0 から 32767 までの整数でなくてはなりません。") 
        && exit;
}
...
sub validateProduct
{
  ...
  grep($_ eq $::FORM{'product'}, @products)
    || DisplayError("存在するプロダクトを選択する必要があります。") 
      && exit;
}
