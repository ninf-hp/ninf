...
unless ( Param("move-enabled") ) {
  print "\n<P>すみません、バグレポートの移行はここでは不可となっています。バグを移行させたい場合は、 " . Param("maintainer") . "に問い合わせてください。";
  exit;
}
...
if ( !defined $::FORM{'buglist'} ) {
  print "Content-type: text/html; charset=EUC-JP\n\n";
  PutHeader("バグの移行");
  print "バグの移行は、バグ表示ページか、<A HREF=\"query.cgi\">検索ページ</A> で検索して「複数バグをまとめて変更」で実行できます。もし移行ボタンがないのなら、ログインしていないか、移行は許可されていないかが理由です。";
  PutFooter();
  exit;
}
...
open(SENDMAIL,
  "|/usr/lib/sendmail -f bugzilla-daemon\@m.aist.go.jp -ODeliveryMode=background -t -i") ||
