...
@::default_column_list = ("severity", "priority", "os", "owner",
...
sub ValidatePassword {
    ...
    if ( length($password) < 3 ) {
        return "パスワードが 3 文字未満になっています。少なくとも 3 文字は必要です。";
    } elsif ( length($password) > 16 ) {
        return "パスワードが 16 文字以上になっています。16文字未満にしてください。";
    } elsif ( $matchpassword && $password ne $matchpassword ) { 
        return "二つのパスワードが一致しません。";
    }
...
sub DBNameToIdAndCheck {
    ...
    ThrowUserError("ユーザ名 <TT>$name</TT> が正しくありません。スペルをまちがえたか、その人物が Bugzilla アカウントを取得していません。");
}
...
sub quoteUrls {
    ...
    my $protocol = join '|',
    qw(afs cid ftp gopher http https mid news nntp prospero telnet wais view-source);
    ...
    while ($text =~ s/\bbug\-org(\s|%\#)*(\d+),?\s*comment\s*(\s|%\#)(\d+)/"##$count##"/ei) {
        my $item = $&;
        my $bugnum = $2;
        my $comnum = $4;
        $item = qq{<A HREF="http://bugzilla.mozilla.org/show_bug.cgi?id=$bugnum">$item</A>};
    ...
    while ($text =~ s/\bbug\-org(\s|%\#)*(\d+)/"##$count##"/ei) {
        my $item = $&;
        my $num = $2;
        $item = qq{<A HREF="http://bugzilla.mozilla.org/show_bug.cgi?id=$num">$item</A>};
        $things[$count++] = $item;
    }
    ...
    while ($text =~ s/\b(Created an )?attachment\-org(\s|%\#)*(\(id=)?(\d+)\)?/"##$count##"/ei) {
        my $item = $&;
        my $num = $4;
        $item = value_quote($item); # Not really necessary, since we know
                                    # there's no special chars in it.
        $item = qq{<a href="http://bugzilla.mozilla.org/attachment.cgi?id=$num&amp;action=view">$item</a>};
        $things[$count++] = $item;
    }
...
sub RemoveVotes {
    ...
    if (0 < @list) {
        foreach my $ref (@list) {
            ...
            if (open(SENDMAIL, "|/usr/lib/sendmail -f bugzilla-daemon\@m.aist.go.jp $sendmailparm -t -i")) {
                my %substs;
...
$::template ||= Template->new(
  {
    ...
    INCLUDE_PATH => "template/ja/custom:template/ja/default:template/en/custom:template/en/default" ,
...
$Template::Stash::SCALAR_OPS->{ truncate } = 
  sub {
      ...
      # http://bugzilla.mozilla.gr.jp/show_bug.cgi?id=1659
      # 日本語が文字化けする対策
      my $v = substr($string, 0, $strlen);
      if ($v =~ /\x8F$/ or $v =~ tr/\x8E\xA1-\xFE// % 2) {
          $strlen--;
      }
...
sub ValidateOutputFormat {
    ...
    if (!$formats->{$format}) {
        ...
        DisplayError("このスクリプトは <em>$escapedname</em> 出力フォーマットをサポートしていません。サポートされているフォーマットは (デフォルトの HTML フォーマットに加え)、 <em>" . 
