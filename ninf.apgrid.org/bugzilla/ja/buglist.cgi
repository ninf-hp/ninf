...
if ($dotweak) {
    ...
    if (!UserInGroup("editbugs")) {
        DisplayError("すみません、あなたには複数のバグを変更するための権限がありません。");
        exit;
    }
    GetVersionTable();
}
...
if ($::FORM{'regetlastlist'}) {
    if (!$::COOKIE{'BUGLIST'}) {
        DisplayError(qq|申しわけありません。以前の検索結果が記録されたクッキーを失ってしまったようです。<a href="query.cgi">検索ページ</a> からやり直してください。|);
        exit;
    }
...
if ($::buffer =~ /&cmd-/) {
    ...
    $vars->{'title'} = "検索ページに項目を追加中...";
    $vars->{'url'} = $url;
    $vars->{'link'} = "自動的に再表示されないときは、ここをクリックしてください。";
...
sub SqlifyDate {
    ...
    if (!defined($date)) {
        ...
        DisplayError("文字列 <tt>$htmlstr</tt> は正しい日付ではありません");
        exit;
    }
...
sub LookupNamedQuery {
    ...
    if (!$result) {
        ...
        DisplayError("<em>$qname</em> という名前の検索条件はもはや存在しないと思われます。");
        exit;
    }
    return $result;
}
...
sub GetQuip {
    ...
    $quip ||= "Bugzilla はランダムに選んだヘッドラインをここに表示します。しかし誰も入力していません。";
    ...
    if (defined $F{'votes'}) {
        ...
        if ($c ne "") {
            if ($c !~ /^[0-9]*$/) {
                ...
                DisplayError("<em>最低 ___ 票</em> という項目は単なる数字でなければいけません。あなたの入力した <kbd>$htmlc</kbd> は正しくありません。");
                exit;
            }
...
#    }
    ...
    foreach my $id ("1", "2") {
        ...
        if (@clist) {
            ...
            DisplayError("<tt>$htmlemail</tt> を検索するためには、その中にある項目を一つかそれ以上選択する必要があります。");
            exit;
        }
    }
    ...
    if (defined $F{'changedin'}) {
        ...
        if ($c ne "") {
            if ($c !~ /^[0-9]*$/) {
                my $htmlc = html_quote($c);
                DisplayError("<em>過去 ___ 日以内の変更</em> 項目は単なる数字でなければいけません。あなたの入力した <kbd>$htmlc</kbd> は正しくありません。");
                exit;
            }
         ...
         "^keywords," => sub {
             ...
             foreach my $value (split(/[\s,]+/, $v)) {
                 ...
                 if ($id) {
                     ...
                     DisplayError(qq|<code>$htmlv</code> という名前のキーワードは存在しません。キーワードで検索するには、<a href="describekeywords.cgi">登録されたキーワードのリスト</a>を考慮してみてください。|);
                     exit;
                 }
             }
             my $haveawordterm;
...
CMD: for ($::FORM{'cmdtype'}) {
    ...
    /^editnamed$/ && do {
        ...
        print "Content-Type: text/html; charset=EUC-JP\n\n";
        # Generate and return the UI (HTML page) from the appropriate template.
        $vars->{'title'} = "$::FORM{'namedcmd'} という名前の検索条件を読み込み中";
        $vars->{'url'} = $url;
        $vars->{'link'} = "自動的に再表示されないときは、ここをクリックしてください。";
        $template->process("global/message.html.tmpl", $vars)
          || ThrowTemplateError($template->error());
    ...
    /^forgetnamed$/ && do {
        ...
        print "Content-Type: text/html; charset=EUC-JP\n\n";
        # Generate and return the UI (HTML page) from the appropriate template.
        $vars->{'title'} = "検索条件は削除されました";
        $vars->{'message'} = "検索条件 <b>$::FORM{'namedcmd'}</b> は削除されました。";
        $vars->{'url'} = "query.cgi";
        $vars->{'link'} = "検索ページに戻る";
        $template->process("global/message.html.tmpl", $vars)
          || ThrowTemplateError($template->error());
    ...
    /^asdefault$/ && do {
        ...
        print "Content-Type: text/html; charset=EUC-JP\n\n";
        # Generate and return the UI (HTML page) from the appropriate template.
        $vars->{'title'} = "デフォルトの検索条件をセットしました";
        $vars->{'message'} = "デフォルトの検索条件をセットしました。個々の検索結果をブックマークすることもできます。";
        $vars->{'url'} = "query.cgi";
        $vars->{'link'} = "検索ページに戻って、新たなデフォルト検索条件を使用する。";
        $template->process("global/message.html.tmpl", $vars)
          || ThrowTemplateError($template->error());
        exit;
    };
    ...
    /^asnamed$/ && do {
        ...
        $name
          || DisplayError("検索条件の名前を入力してください。")
            && exit;
        $name =~ /[<>&]/
          && DisplayError("検索条件の名前に以下で示す文字を含むことはできません: &lt;, &gt;, &amp;.")
            && exit;
        my $qname = SqlQuote($name);
        ...
        $vars->{'title'} = "検索条件は保存されました";
        $vars->{'message'} = "検索条件 <code>$name</code> は保存されました。";
        $vars->{'url'} = "query.cgi";
        $vars->{'link'} = "検索ページに戻る";
        $template->process("global/message.html.tmpl", $vars)
          || ThrowTemplateError($template->error());
