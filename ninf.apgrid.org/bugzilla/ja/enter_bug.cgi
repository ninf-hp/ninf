...
if (!defined $::FORM{'product'}) {
    ...
    if ($prodsize == 0) {
        DisplayError("バグの対象プロダクトがまったく登録されていません。またはあなたにはどのプロダクトへのアクセス権もありません。\n");
        exit;
    } 
    elsif ($prodsize > 1) {
...
if(Param("usebuggroupsentry") 
   ...
   && !UserInGroup($product)) 
{
    DisplayError("申しわけありません。あなたにはこのプロダクトのバグを登録する権限がありません。\n");
    exit;
}
...
if (lsearch(\@::enterable_products, $product) == -1) {
    DisplayError("'" . html_quote($product) . "' は正しいプロダクトではありません。");
    exit;
}
    
if (0 == @{$::components{$product}}) {
    my $error = "申しわけありません。バグレポートを登録するためには、このプロダクトに少なくとも一つのコンポーネントが必要です。 ";
    if (UserInGroup('editcomponents')) {
        $error .= "<a href=\"editcomponents.cgi\">" . 
                  "新規コンポーネントを作成してください</a>\n";
    }
    else {              
        $error .= Param("maintainer") . "にあなたが登録しようとしていたプロダクトについて相談してください。\n";
    }
