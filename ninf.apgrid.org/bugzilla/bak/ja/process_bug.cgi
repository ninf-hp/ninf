...
sub CheckonComment( $ ) {
    ...
    if( $ret ) {
        if (!defined $::FORM{'comment'} || $::FORM{'comment'} =~ /^\s*$/) {
            ...
            ThrowUserError("この変更についての <b>コメント</b> を記入してください。変更した理由を簡潔に述べます。");
        } else {
            $ret = 0;
        }
    }
...
sub CheckCanChangeField {
    ...
    ThrowUserError("<strong>$f</strong> の項目を <em>$oldvalue</em> から <em>$newvalue</em> に変更できるのは、担当者か報告者、またはその権限のあるユーザのみです。");
}
...
SWITCH: for ($::FORM{'knob'}) {
    ...
    /^reassign$/ && CheckonComment( "reassign" ) && do {
        ...
        if ( !defined$::FORM{'assigned_to'} ||
          ...
          ThrowUserError("バグを再割り当てする人を指定してください。「バグを .. に再割り当てする」を意図的に空白にしたわけではないなら、以下の理由かもしれません: " . Param("browserbugmessage"));
        }
    ...
    /^reassignbycomponent$/  && CheckonComment( "reassignbycomponent" ) && do {
        if ($::FORM{'product'} eq $::dontchange) {
            ThrowUserError("プロダクトを選択してください。このバグの新担当者を決めるために必要です。");
        }
        if ($::FORM{'component'} eq $::dontchange) {
            ThrowUserError("コンポーネントを選択してください。そのコンポーネントの初期担当者が、このバグを担当することになります。");
        }
        if ($::FORM{'compconfirm'}) {
            DoConfirm();
        }
    ...
    /^duplicate$/ && CheckonComment( "duplicate" ) && do {
        ...
        $num = FetchOneColumn();
        if (!$num) {
            ThrowUserError("このバグと重複しているバグの番号を正しく入力してください。バグレポートは変更されませんでした。")
        }
        if (!defined($::FORM{'id'}) || $num == $::FORM{'id'}) {
            ThrowUserError("ナイストライだ、$::COOKIE{'Bugzilla_login'}。だが、バグそれ自身を重複にするのは意味がないんじゃないかな。");
        }
        my $checkid = trim($::FORM{'id'});
        SendSQL("SELECT bug_id FROM bugs where bug_id = " .  SqlQuote($checkid));
        $checkid = FetchOneColumn();
        if (!$checkid) {
            ThrowUserError("Bug #$::FORM{'id'} は正しくありません。");
        }
    ...
    ThrowCodeError("$escaped_knob は不明な動作です。\n");
}


if ($#idlist < 0) {
    ThrowUserError("あなたはバグを一つも選んでいませんね。");
}
...
if ($::FORM{'keywords'}) {
    foreach my $keyword (split(/[\s,]+/, $::FORM{'keywords'})) {
        ...
        if (!$i) {
            ThrowUserError("<code>" . html_quote($keyword) . 
                           "</code> は登録されていないキーワードです。 <p>使用可能なキーワードの<a href=\"describekeywords.cgi\">一覧はここにあります</a>。</p>.");
        }
        if (!$keywordseen{$i}) {
            push(@keywordlist, $i);
            $keywordseen{$i} = 1;
...
if ($::comma eq ""
    ...
    if (!defined $::FORM{'comment'} || $::FORM{'comment'} =~ /^\s*$/) {
        ThrowUserError("バグレポートのどの部分も変更されていません。");
    }
}
...
foreach my $id (@idlist) {
    ...
    if ($requiremilestone) {
        ...
        if ($value eq FetchOneColumn()) {
            SendSQL("UNLOCK TABLES");
            ThrowUserError("バグを引き受ける場合、ターゲットマイルストーンを選択しなくてはなりません。バグを引き受けるということの一部に、バグはいつ修正されるのかを見積ることも含まれています。", 
                           undef, 
                           "abort");
        }
    }   
        ...
        for (1..2) {
            ...
            foreach my $i (split('[\s,]+', $::FORM{$target})) {
                ...
                if (!detaint_natural($i)) {
                    ThrowUserError("$orig は正しいバグ番号ではありません", undef, "abort");
                }
                ...
                if ($comp ne $i) {
                    ThrowUserError("$i は正しいバグ番号ではありません", undef, "abort");
                }
                if ($id eq $i) {
                    ThrowUserError("バグそれ自身の修正を妨害したり依存したりすることはできません。",
                                   undef,
                                   "abort");
                }
                if (!exists $seen{$i}) {
            ...
            if ($me eq 'dependson') {
                ...
                if (@isect > 0) {
                    ...
                    ThrowUserError(qq|依存関係のループが検出されました!<p>
                      もし変更が記録されていれば、以下のバグが依存関係のツリーにおいて、依存し、かつ妨害しあうことになります。: $both<br>依存関係の循環になるところでした。これは、あってはならないことです。</p>|,
                      undef,
                      "abort");
                }
            }
