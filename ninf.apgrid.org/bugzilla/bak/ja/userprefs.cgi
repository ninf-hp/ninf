...
sub SaveAccount {
        ...
        $pwd1 ne "" || $pwd2 ne "") 
    {
        ...
        if (!$oldcryptedpwd) {
            DisplayError("あなたの旧パスワードをデータベースから取得することができません。");
            exit;
        }
        if (crypt($::FORM{'Bugzilla_password'}, $oldcryptedpwd) ne 
                  $oldcryptedpwd) 
        {
            DisplayError("入力された旧パスワードが正しくありません。");
            exit;
        }
        ...
        {
            if ($pwd1 ne $pwd2) {
                DisplayError("入力された二つのパスワードが一致しません。");
                exit;
            }
            if ($::FORM{'new_password1'} eq '') {
                DisplayError("新パスワードを入力してください。");
                exit;
            }
    ...
    if(Param("allowemailchange") && $::FORM{'new_login_name'}) {
        ...
        if($old_login_name ne $new_login_name) {
            if( $::FORM{'Bugzilla_password'} eq "") {
                DisplayError("メールアドレスを変更するためには、あなたの旧パスワードを入力する必要があります。");
                exit;
            }
            ...
            if (Token::HasEmailChangeToken($userid)) {
                DisplayError("メールアドレスの変更は既に進行中です。メールをチェックしてください。");
                exit;
            }
            ...
            if (!ValidateNewUser($new_login_name)) {
                DisplayError("アカウント $new_login_name は既に存在します");
                exit;
            }
            ...
            $vars->{'changes_saved'} = 
                "メールアドレスの変更を確認するために、旧アドレスと新アドレスの両方にメールが送信されました。";
        }
    }
...
sub SaveFooter {
    ...
    for (my $c = 0; $c < $::FORM{'numqueries'}; $c++) {
        ...
        if (exists $old{$name}) {
            ...
            DisplayError("うーむ、検索条件 $name は消えてしまったようです。");
        }
    }
