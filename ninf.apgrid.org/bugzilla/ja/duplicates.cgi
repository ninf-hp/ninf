...
if (!tie(%dbmcount, 'AnyDBM_File', "data/duplicates/dupes$today",
    ...
    if ($!{ENOENT}) {
        if (!tie(%dbmcount, 'AnyDBM_File', "data/duplicates/dupes$yesterday",
                 O_RDONLY, 0644)) {
            if ($!{ENOENT}) {
                ThrowUserError("今日 ($today) と昨日は、重複はありません。",
                               "重複はありません");
            } else {
                ThrowUserError("今日 ($today) は、重複はありません。昨日の重複ファイルにアクセスするとき、エラーが発生しました: $!.",
                               "昨日の重複ファイル読み込み時のエラー");
            }
        }
    } else {
        ThrowUserError("今日の重複ファイルにアクセスするとき、エラーが発生しました: $!.",
                       "今日の重複ファイル読み込み時のエラー");
    }
}
...
if (!tie(%before, 'AnyDBM_File', "data/duplicates/dupes$whenever",
    ...
    if (!$!{ENOENT}) {
        ThrowUserError("$changedsince 日前 ($whenever) の重複ファイルを読み込みできません: $!",
                       "以前の重複ファイル読み込み時のエラー");
    }
} else {
