...
if (Param("shutdownhtml") && $0 !~ m:(^|[\\/])(do)?editparams\.cgi$:) {
    ...
    $::vars->{'title'} = "Bugzilla は動作を停止しています";
    $::vars->{'h1'} = "Bugzilla は動作を停止しています";
...
sub ParseUrlString {
    ...
    foreach my $arg (@args) {
        ...
        # 半角カタカナを全角に変換
        my $jcode = eval("require 'jcode.pl'");
        if ($jcode) {
            my $ic = &jcode::getcode(\$value);
            &jcode::convert(\$value, 'euc', $ic, 'z');
        }
...
sub CheckFormFieldDefined (\%$) {
    ...
    if (!defined $formRef->{$fieldname}) {
          ThrowCodeError("$fieldname は定義されていません; " . 
...
sub ValidateBugID {
    ...
    detaint_natural($_[0])
      || DisplayError("バグ番号が正しくありません。もし QuickSearch を使用しているのなら、ブラウザの JavaScript を有効にする必要があります。この制限の修正については、<a href=\"http://bugzilla.mozilla.org/show_bug.cgi?id=70907\">こちら</a>を見てください。") 
      && exit;
    ...
    if ($::userid) {
        DisplayError("Bug #$id にアクセスする権限がありません。");
    } else {
        DisplayError(
          qq|Bug #$id にアクセスする権限がありません。まずは、適切な権限をもったアカウントに<a href="show_bug.cgi?id=$id&amp;GoAheadAndLogIn=1">ログイン</a>してください。|
        );
    }
    exit;
...
sub ValidateComment {
    ...
    if (defined($comment) && length($comment) > 65535) {
        DisplayError("コメントは 65,535 文字より長くは入力できません。");
        exit;
    }
}
...
sub make_options {
    ...
    if (!$found && $default ne "") {
      if ( $::CheckOptionValues &&
        ...
        print "バグデータベースが壊れている可能性が検出されました。" . Param("maintainer") . " にメールして、何をしているときにこのメッセージが現れたか詳しく説明してください。ご協力ありがとうございます。\n";
        if (!$src) {
            $src = ["???null???"];
        }
...
sub CheckEmailSyntax {
    ...
    if ($addr !~ /$match/ || $addr =~ /[\\\(\)<>&,;:"\[\] \t\r\n]/) {
        ThrowUserError("あなたの入力した メールアドレス (<b>" .
        html_quote($addr) . "</b>) は正しいメールアドレスの文法にしたがっていません。<br>" . Param('emailregexpdesc') . '<br> また、以下に示す特殊な文字のどれも含むべきではありません:<br> <tt>\ ( ) &amp; &lt; &gt; , ; : " [ ]</tt> または空白文字<br>メールアドレスの文法をチェックしてください。');
    }
}
...
sub MailPassword {
    ...
    my $mime = eval("require 'mimew.pl'");
    if ($mime) {
        $mailhead = &::mimeencode($mailhead);
    }
    my $jcode = eval("require 'jcode.pl'");
    if ($jcode) {
        &jcode::euc2jis(\$mailbody);
    }
    $msg = $mailhead . "\n\n" . $mailbody;

    open SENDMAIL, "|/usr/lib/sendmail -f bugzilla-daemon\@m.aist.go.jp -t -i";
    print SENDMAIL $msg;
    close SENDMAIL;
}
...
#    print "Content-type: text/plain\n\n";
    ...
    if (defined $::FORM{"Bugzilla_login"} && defined $::FORM{"Bugzilla_password"}) {
        ...
        $userid
          || DisplayError("ユーザ名またはパスワードが正しくありません。")
          && exit;
        ...
        if ( defined $::FORM{"PleaseMailAPassword"} && !$userid ) {
            ...
            if(!ValidateNewUser($enteredlogin)) {
                ThrowUserError("そのアカウントは既に存在しています。");
            }
        ...
        # Otherwise, authenticate the user.
            ...
            while ( $token ) {
                Token::Cancel($token, "ユーザがログインしました");
...
#       }
       ...
       if ($LDAPserver eq "") {
         print "Content-type: text/html; charset=EUC-JP\n\n";
         PutHeader("LDAP サーバが定義されていません");
         print "認証のための LDAP サーバが定義されていません。  ";
         print Param("maintainer")." にこの問題について知らせてあげてください。\n";
         PutFooter();
         exit;
       }
       ...
       if(!$LDAPconn) {
         print "Content-type: text/html; charset=EUC-JP\n\n";
         PutHeader("LDAP サーバと接続できません");
         print "LDAP サーバと接続できないため、ユーザ認証できません。";
         print Param("maintainer");
         print " にこの問題について知らせてあげてください。\n";
         PutFooter();
         exit;
       }
       ...
       if ( ! $::FORM{"LDAP_password"} ) {
         print "Content-type: text/html; charset=EUC-JP\n\n";
         PutHeader("ログインに失敗しました");
         print "パスワードが記入されていません。\n";
         print "<b>Back</b> を押して、やりなおしてください。\n";
         PutFooter();
         exit;
       }
       ...
       if(!$userEntry->exists(Param("LDAPmailattribute"))) {
         print "Content-type: text/html; charset=EUC-JP\n\n";
         PutHeader("LDAP の認証エラーです");
         print "LDAP サーバから ".Param("LDAPmailattribute");
         print " 属性を取得することができませんでした。";
         print Param("maintainer")." にこの問題について知らせてあげてください。\n";
         PutFooter();
         exit;
       }
...
";
            ...
            "あなたのアカウントは回復されるべきであると思われるのであれば、" .
            Param("maintainer") . " までメールして、アカウントが使用不可となった理由を説明してもらってください");
        }
        print "Content-type: text/html; charset=EUC-JP\n\n";
        PutHeader("ログイン");
        if(Param("useLDAP")) {
          print "続けるには、正しい LDAP ユーザ名とパスワードが必要です。\n";
        } else {
          print "続けるには、正しいメールアドレスとパスワードが必要です。\n";
        }
...
";
        ...
        print qq|
          <input type="submit" name="GoAheadAndLogIn" value="ログイン">
          </form>
        |;
        ...
        unless( Param("useLDAP") ) {
            print qq|
              <hr>
              <p>Bugzilla アカウントをまだ持っていないのであれば、<a href="createaccount.cgi">新規アカウントを作成</a>することができます。</p>
              <form method="get" action="token.cgi">
                <input type="hidden" name="a" value="reqpw">
                もしアカウントを持っているけれどパスワードを忘れた場合、以下にログイン名を入力して、パスワード変更リクエストを押してください。<br>
                <input size="35" name="loginname">
                <input type="submit" value="パスワード変更リクエスト">
              </form>
              <hr>
            |;
        }
...
sub ThrowTemplateError {
    ...
    if (!$template->process("global/code-error.html.tmpl", $vars)) {
        ...
        <tt>
          <p>
    Bugzilla は内部エラーによりダメージを受けました。このページを保存して、
    [% Param("maintainer") %] 宛に、このメッセージが現れたとき何をしていたのか
    詳しく教えてください。
          </p>
          <script> <!--
          ...
          <p>Template->process() は 2回失敗しました。<br>
          最初のエラー : $error<br>
          2度目のエラー: $error2</p>
        </tt>
END
