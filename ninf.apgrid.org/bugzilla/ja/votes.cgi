...
if ($action eq "show_bug") {
    ...
    DisplayError("不明な命令です: " . html_quote($action));
}
...
sub show_user {
    ...
    if (!$who) {
        DisplayError(html_quote($name) . " は正しいユーザ名ではありません。\n");
        exit;
    }
...
sub record_votes {
    ...
    foreach my $id (@buglist) {
      ...
      detaint_natural($::FORM{$id})
        || DisplayError("バグへの投票に負の数は使用できません。")
        && exit;
    }
    ...
    if (scalar(@buglist)) {
        ...
        while (MoreSQLData()) {
            ...
            if ($::FORM{$id} > $max) {                
                ...
                DisplayError("<tt>$prod</tt> プロダクトの個々のバグには、最大 $max 票までしか投票できません。しかし、あなたは $votes 票も投票しようとしました。", "Illegal vote");
                exit();
            } 
        }
        ...
        foreach my $prod (keys(%prodcount)) {
            if ($prodcount{$prod} > $::prodmaxvotes{$prod}) {
                ...
                DisplayError("バグレポートがいくらあっても、<tt>$prod</tt> プロダクトには合計で、$::prodmaxvotes{$prod} 票までしか投票できません。しかし、あなたは $prodcount{$prod} 票も投票しようとしました。", "Illegal vote");
                exit();
            }
        }
    }
