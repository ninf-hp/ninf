...
if (! defined $FORM{'product'}) {
    ...
    Param("usebuggroups") 
      ...
      && DisplayError("このプロダクトの統計を見る権限がありません。")
      && exit;
    ...
    if ($FORM{'banner'}) {
      ...
      print("<html><head><title>バグの統計</title></head><body bgcolor=\"#FFFFFF\">");
    }
...
sub choose_product {
    ...
    # Can we do bug charts?  
     ...
     my $charts = $do_charts ? "<option value=\"show_chart\">グラフ" : "";
 
    print <<FIN;
<center>
<h1>Bugzilla Query Kitchen へようこそ</h1>
<form method=get action=reports.cgi>
<table border=1 cellpadding=5>
<tr>
<td align=center><b>プロダクト:</b></td>
<td align=center>
<select name="product">
$product_popup
</select>
...
<tr>
<td align=center><b>出力形式:</b></td>
<td align=center>
<select name="output">
<option value="most_doomed">バグ件数
FIN
    if (Param('usetargetmilestone')) {
        print "<option value=\"most_doomed_for_milestone\">不運な人";
    }
    print "<option value=\"most_recently_doomed\">最近の不運な人";
    print <<FIN;
$charts
</select>
</tr>
FIN
  ...
  if ($do_charts) {
      print <<FIN;
      <tr>
      <td align=center><b>グラフに<br>表示する要素:</b></td>
      <td align=center>
...
print <<FIN;
<tr>
<td align=center><b>表示の設定:</b></td>
<td align=left>
<input type=checkbox name=links checked value=1>&nbsp;バグにリンクする<br>
<input type=checkbox name=banner checked value=1>&nbsp;バナー表示<br>
FIN
  
    if (Param('usequip')) {
        print "<input type=checkbox name=quip value=1>&nbsp;ヘッドライン表示<br>";
    } else {
...
<table border=1 cellpadding=5>
<tr>
<td align=right><b>今週の登録件数</b></td>
<td align=center>$bugs_new_this_week</td>
</tr>

<tr>
<td align=right><b>New バグ</b></td>
<td align=center>$bugs_status{'NEW'}</td>
</tr>

<tr>
<td align=right><b>Assigned バグ</b></td>
<td align=center>$bugs_status{'ASSIGNED'}</td>
</tr>

<tr>
<td align=right><b>Reopened バグ</b></td>
<td align=center>$bugs_status{'REOPENED'}</td>
</tr>

<tr>
<td align=right><b>合計件数</b></td>
...
<tr>
<td align=center bgcolor="#DDDDDD"><b>担当者</b></td>
...
<td align=center bgcolor="#DDDDDD"><b>合計</b></td>
</tr>
FIN
...
sub most_doomed_for_milestone {
    ...
    if( $FORM{'product'} ne "-All-" ) {
        ...
        $ms = FetchOneColumn();
        print "マイルストーン $ms ($FORM{'product'}) の不運な人";
    } else {
        print "マイルストーン $ms の不運な人";
    }
    print "</h1>\n$when<p>\n";
...
#   sort people by the number of bugs they have assigned to this milestone
    ...
    print "$totalpeople 人の担当者が $bugtotal 個の $ms バグをかかえています。\n";
    print "</TD></TR>\n";
...
sub most_recently_doomed {
    ...
    # Build up $query string
    my $query = "select distinct assigned_to from bugs where bugs.bug_status='NEW' and bug_severity!='enhancement' and status_whiteboard=''";
    if ($FORM{'product'} ne "-All-" ) {
        $query .= "and    bugs.product=".SqlQuote($FORM{'product'});
    }
...
# End build up $query string
    ...
    my $bugtotal = 0;
    foreach $person (@people) {
        my $query = "select count(bug_id) from bugs,profiles where bugs.bug_status='NEW' and userid=assigned_to and userid='$person' and bug_severity!='enhancement' and status_whiteboard=''";
        if( $FORM{'product'} ne "-All-" ) {
...
#   sort people by the number of bugs they have assigned to this milestone
    ...
    print "<TABLE>\n";
    print "<TR><TD COLSPAN=2>\n";
    print "$totalpeople 人の担当者が $bugtotal 個の NEW バグに手をつけていません。\n";
    if ($totalpeople > 20) {
        print "以下はそのうちの 20 人です。";
    }
    print "</TD></TR>\n";
    ...
    while (@people) {
        ...
        SendSQL("select login_name from profiles where userid=$person");
        my $login_name= FetchSQLData();
        print("<A HREF=\"buglist.cgi?bug_status=NEW&email1=$login_name&emailtype1=substring&emailassigned_to1=1&status_whiteboard=.&status_whiteboard_type=notregexp&bug_severity=blocker&bug_severity=critical&bug_severity=major&bug_severity=normal&bug_severity=minor&bug_severity=trivial\">\n"); 
